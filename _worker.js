    ...
    async fetch(request, env, ctx) {
        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                ...
                const 指定用户 = params.get('user') || userID;
                params.set('user', 指定用户);
                params.set('token', await MD5MD5(host + 指定用户));
                ...
                } else if (访问路径 === 'admin/ui') {
                    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>用户管理与订阅</title><style>body{font-family:system-ui,Segoe UI,Arial;margin:24px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccc;padding:8px;text-align:left}input,button,select{padding:6px;margin:4px}code{background:#f5f5f5;padding:2px 4px;border-radius:3px}</style></head><body><h2>用户管理与订阅</h2><div style="display:flex;gap:24px;align-items:flex-start;flex-wrap:wrap"><div style="flex:1;min-width:420px"><h3>用户列表</h3><div><button id="refresh">刷新列表</button></div><table id="list"><thead><tr><th>UUID</th><th>到期</th><th>禁用</th><th>备注</th><th>操作</th></tr></thead><tbody></tbody></table></div><div style="flex:1;min-width:420px"><h3>订阅生成</h3><div><label>选择或输入 UUID</label><br/><input id="sub-uuid" style="width:380px" placeholder="uuid"/><br/><button id="get-token">获取 Token</button><br/><div id="token-box" style="margin-top:8px"></div><div id="links" style="margin-top:8px"></div></div><h3 style="margin-top:24px">创建/更新用户</h3><div><label>UUID(留空自动生成)</label><br/><input id="uuid" style="width:380px"/><br/><label>到期时间(ISO 或 毫秒)</label><br/><input id="expires" style="width:380px"/><br/><label>禁用</label><br/><select id="disabled"><option value="false">否</option><option value="true">是</option></select><br/><label>备注</label><br/><input id="note" style="width:380px"/><br/><label>Trojan密码(可选)</label><br/><input id="trojan" style="width:380px"/><br/><button id="save">保存</button></div></div></div><script>function buildRow(x){return '<td>'+((x.uuid)||'')+'</td><td>'+((x.expiresAt)||'')+'</td><td>'+(x.disabled?'是':'否')+'</td><td>'+((x.note)||'')+'</td><td><button data-id="'+((x.uuid)||'')+'" class="del">删除</button><button data-copy="'+((x.uuid)||'')+'" class="copy" style="margin-left:6px">复制订阅</button></td>'}function buildLinks(origin,user,token){var base=origin+'/sub?user='+encodeURIComponent(user)+'&token='+encodeURIComponent(token);var html='订阅链接：<br/><code>'+base+'</code><br/>';html+=('<a href="'+base+'&target=clash" target="_blank">Clash</a> | ');html+=('<a href="'+base+'&target=singbox" target="_blank">Singbox</a> | ');html+=('<a href="'+base+'&target=surge" target="_blank">Surge</a> | ');html+=('<a href="'+base+'&target=mixed" target="_blank">Mixed</a>');return html}async function load(){const r=await fetch('/admin/api/users');const arr=await r.json();const tb=document.querySelector('#list tbody');tb.innerHTML='';(arr||[]).forEach(function(x){const tr=document.createElement('tr');tr.innerHTML=buildRow(x);tb.appendChild(tr);});tb.querySelectorAll('.del').forEach(function(b){b.onclick=async function(){const id=b.getAttribute('data-id');const r=await fetch('/admin/api/users/'+id,{method:'DELETE'});if(r.ok)load();}});tb.querySelectorAll('.copy').forEach(function(b){b.onclick=async function(){var id=b.getAttribute('data-copy');document.getElementById('sub-uuid').value=id;await getTokenAndLinks();}});}async function getTokenAndLinks(){var user=document.getElementById('sub-uuid').value||'';if(!user){alert('请输入或选择一个 UUID');return}const r=await fetch('/admin/api/token?user='+encodeURIComponent(user));if(!r.ok){alert('获取 token 失败');return}const j=await r.json();var token=j.token;document.getElementById('token-box').innerHTML='Token: <code>'+token+'</code>';document.getElementById('links').innerHTML=buildLinks(location.origin,user,token);}document.getElementById('refresh').onclick=load;document.getElementById('get-token').onclick=getTokenAndLinks;document.getElementById('save').onclick=async function(){const uuid=document.getElementById('uuid').value||undefined;const expires=document.getElementById('expires').value||undefined;const disabled=document.getElementById('disabled').value==='true';const note=document.getElementById('note').value||undefined;const trojanPassword=document.getElementById('trojan').value||undefined;const r=await fetch('/admin/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uuid,expiresAt:expires,disabled,note,trojanPassword})});if(r.ok){alert('保存成功');load();}else{alert('保存失败')}};load();</script></body></html>`;
                    return new Response(html, { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
